<x-main>
    <div class="container mt-5">
        ...
        <form method="POST" action="{{ route('course.store') }}" enctype="multipart/form-data">
            ...
            <div class="form-floating mb-3"><input  value="{{ old('info') }}" name="info" autocomplete="on" class="form-control " id="floatingInput" type="text" placeholder="Info" /><label for="floatingInput">Info</label></div>
            <div class="form-floating mb-3"><input value="{{old('prise')}}" name="prise" oninput="formatNumbers(this)" class="form-control  @error('prise') is-invalid @enderror" id="floatingInput" type="text" placeholder="Narxi" /><label for="floatingInput">Narxi</label>
                @error('prise')
                <p class="text-danger">To'ldirish shart va Faqat raqam kriting</p>
                @enderror
            </div>
            <div class="form-floating mb-3"><input value="{{old('days')}}" name="days" class="form-control  @error('days') is-invalid @enderror" id="floatingInput" type="number" placeholder="Sharif" /><label for="floatingInput">Bir oyda necha kun?</label>
                @error('days')
                <p class="text-danger">To'ldirish shart</p>
                @enderror
            </div>
